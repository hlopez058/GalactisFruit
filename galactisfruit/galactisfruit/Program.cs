...
namespace galactisfruit
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            var depth = 210; // multiplied by z to crete roundness
            var octaves = 10;//frequency of noise
            var persistence = .8;//amplitude of frequency
            
            //create surface model
            var globe = new Surface(origin, radius, depth, octaves, persistence);
            ...
            for (int x = 0; x < img_size; x++)
            {
                ...
                {
                    //add noise from surface model to pixel map
                    map[x, y] = globe.GetPointVal(x, y);
                }
            }
    ...
    public class Surface
    {
        ...
        internal double GetPointVal(int x, int y)
        {
            ...
            if (this.TestPoint(x, y))
            {
                ...
                return Noise.scaleBetween(n, 0, this.radius);
            }
            else
            {
                return 0;
